    ...
    private static int solution(String[][] clothes) {
        ...
        for(String[] tmp : clothes) {
            ...
            if(clothMap.containsKey(key)) {
                clothMap.replace(key, clothMap.get(key)+1);
            } else {
                clothMap.put(key, 1);
            }
        }

        Iterator keys= clothMap.keySet().iterator();

        while(keys.hasNext()) {
            String key = (String) keys.next();
            answer *= (clothMap.get(key)+1);
        }
        // 경우의 수를 살펴보면 A,B,C 세 카테고리가 있다고 쳤을 때
        // 가능한 경우는 A,B,C,AB,AC,BC,ABC 이다.
        // 따라서 모든 카테고리마다 착용을 안하는 경우를 추가하는 +1을 해주고
        // 모두 착용하지 않은 1가지 경우싀 수를 빼기 위해 -1을 해준다.
