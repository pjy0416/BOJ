...
public class ColumnAndBeam_2019 {
    static int[][][] map;
    final static int COLUMN =0, BEAM =1, ADD =1, REMOVE =0;
    private static int[][] solution(int n, int[][] build_frame) {
        ...
        for(int[] cmd : build_frame) {
            ...
            // 1 : y
            // 2 : 종류 (0 : 기둥 or 1 : 보)
            // 3 : 삭제 or 생성(0: 삭제, 1: 생성)
            cnt = doCmd(cmd, cnt);
        }
        print(n);

        return getResult(n,cnt);
    }

    private static int[][] getResult(int n, int cnt) {
        int[][] result = new int[cnt][];
        int idx=0;
        for(int x=1; x<=n+1; x++) {
            for(int y=1; y<=n+1; y++) {
                if(map[x][y][COLUMN] ==1) {
                    int[] tmp = {x-1,y-1, COLUMN};
                    result[idx++] = tmp;
                }
                if(map[x][y][BEAM] ==1) {
                    int[] tmp = {x-1,y-1, BEAM};
                    result[idx++] = tmp;
                }
            }
        }
    ...
    private static int doCmd(int[] cmd, int cnt) {
        int x = cmd[0] +1;
        int y = cmd[1] +1;
    ...
    private static boolean canMakeBeam(int x, int y) { // 보
        boolean result = false;
        if(map[x][y-1][COLUMN] == 1 || map[x+1][y-1][COLUMN] ==1) { // 한쪽 끝 부분이 기둥 위에 있거나,
            result = true;
        }
        if(map[x-1][y][BEAM] == 1 && map[x+1][y][BEAM] ==1) {    // 또는 양쪽 끝 부분이 다른 보와 동시에 연결되어 있어야 합니다.
            result = true;
        }
    ...
    private static boolean canMakeColumn(int x, int y) { // 기둥
        boolean result = false;
        if(y == 1) {     // 바닥 위에 있거나
            result = true;
        }
        if(map[x][y][BEAM] ==1 || (map[x-1][y][BEAM] ==1)) { // 보의 한쪽 끝 부분 위에 있거나
            result = true;
        }
        if(map[x][y-1][COLUMN] == 1) {  // 또는 다른 기둥 위에 있어야 합니다.
            result = true;
        }
        return result;
    }

    private static boolean canRemoveColumn(int x, int y) {
        boolean result = true;
        if(map[x][y][COLUMN] ==1) {
            map[x][y][COLUMN] = 0;
            if (map[x][y+1][COLUMN] == 1 && !canMakeColumn(x, y + 1)) {  // 위에 기둥이 있는 경우
                result = false;
            } if (map[x][y+1][BEAM] == 1 && !canMakeBeam(x, y + 1)) { // 위에 보가 있는 경우
                result = false;
            }
            if (map[x-1][y+1][BEAM] == 1 && !canMakeBeam(x - 1, y + 1)) { // 좌측 상단에 보가 있는 경우
                result = false;
            }
            map[x][y][COLUMN] = 1;
        }
    ...
    private static boolean canRemoveBeam(int x, int y) {
        map[x][y][BEAM] =0;
        boolean result = true;

        if( (map[x][y][COLUMN] ==1 && !canMakeColumn(x,y))|| (map[x+1][y][COLUMN] == 1) && !canMakeColumn(x+1,y)) {   // 보의 양 끝에 기둥이 있는 경우
            result = false;
        }
        if( (map[x-1][y][BEAM] == 1 && !canMakeBeam(x-1,y)) || (map[x+1][y][BEAM] ==1) && !canMakeBeam(x+1,y)) {    // 양 옆에 보가 있는 경우
            result = false;
        }
...
구조물은 교차점 좌표를 기준으로
보는 오른쪽, 기둥은 위쪽 방향으로 설치 또는 삭제합니다.
